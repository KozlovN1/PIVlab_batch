...
% This procedure does the job for the scan-left-to-right-first case
% Version 1.0
% 2022-10-12
% Nikolai Kozlov
...
for psi_count=1:size(u_filtmean,1)
    ...
    if isnan(bottomBC)
        PSI11(:,1) = straight_y_stroke_1(1,1,jmax,leftBC,topBC,X1,Y1,U1,V1);
        PSI12(:,1) = reverse_y_stroke_1(1,2,jmax,leftBC,PSI11(end,1),X1,Y1,U1,V1);
        for i=2:imax
    ...
    elseif isnan(topBC)
        PSI12(:,1) = reverse_y_stroke_1(1,1,jmax,leftBC,bottomBC,X1,Y1,U1,V1);
        PSI11(:,1) = straight_y_stroke_1(1,2,jmax,leftBC,PSI12(1,1),X1,Y1,U1,V1);
        for i=2:imax
    ...
    if scanpasses==2
        ...
        if isnan(bottomBC)
            if isnan(rightBC)
                PSI21(:,imax)=PSI11(:,imax);
                PSI22(:,imax)=PSI12(:,imax);
            else
